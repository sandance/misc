...
send_files_for_bb()
{
        ssh nazmul@10.255.211.27  "mkdir -p $bb_disk/$osticket/$studyname/{OD,user,pde}"      # create the dirs in remote folder
	
        rsync -avz  $postpde/output/{HomeWorkGroup,HomeWorkAll,Penetration,GenerateTripLegs,GenerateTripLegsSubDAG,GenerateHomeWorkMatrix,GenerateTripLegMatrix,CombineTripLegs,ClassifySubscribers,PackageODStudy,ClassifyPointsDayGroup,ClassifyPointsDay,PackageClassifiedPoints}  nazmul@10.255.211.27:$bb_disk/$osticket/$studyname/OD/                                         #copy od study output, volume=static2/od or ibmlun2 
        rsync -avz  /opt/home/$od_account/atp  nazmul@10.255.211.27:$bb_disk/$osticket/$studyname/user/    #copy user account config files 
	rsync -avz  $pdepath/*  nazmul@10.255.211.27:$bb_disk/$osticket/$studyname/pde/                    # send pde files in BB
