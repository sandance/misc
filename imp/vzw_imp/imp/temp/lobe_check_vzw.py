...
def calculation(cursor,switch):
    cursor.execute("SELECT st_area(ST_ConvexHull(ST_Collect(wkb_geometry))) FROM public.lcib20131016  WHERE sseo_switch_name='%s'" %  switch)
    records = cursor.fetchone()

    cursor.execute("SELECT st_area( st_union) FROM (SELECT ST_union(wkb_geometry) FROM public.lcib20131016  WHERE sseo_switch_name ='%s')a" % switch)
...
def main():
    ...
    cursor.execute("select DISTINCT(sseo_switch_name) from public.lcib20131016")
