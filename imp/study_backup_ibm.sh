...
temp_dir=/mnt/$volume/atp/temp/$ticket
...
send_to_SL_and_BB()
{
	...
	rysnc -a --no-p --no-o --no-g --progress /mnt/$volume/atp/temp/${ticket}  rsync://10.255.211.27:/study/$disk/
	tar --remove-files -cvf /mnt/$volume/atp/temp/${ticket}.tar  /mnt/$volume/atp/temp/$ticket/
	rsync -a  --no-p --no-o --no-g --progress  /mnt/$volume/atp/temp/${ticket}.tar rsync://10.255.211.46/ODSTUDIES/

	echo "study ${ticket} has been sent to 211.41"
	rm /mnt/$volume/atp/temp/${ticket}*
